...
async function main(): Promise<void> {
  ...
  await savePolicy(u.rows[0].id, { ageEscalation: process.argv[3] !== 'off' });
  const p = await getPolicy(u.rows[0].id);
  process.stdout.write(`ageEscalation ${p?.ageEscalation} · minScore ${p?.minScore} · neverAsk ${p?.neverAsk}\n`);
  process.exit(0);
}
